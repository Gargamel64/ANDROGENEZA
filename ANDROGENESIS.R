...
fix_markers <- function(df){
  ...
  # =========================================================
  # ===== 1. znajdź kolumnę z nazwami markerów
  # =========================================================
  marker_col <- intersect(
    c("MARKER","MARKERS","NAZWA_REGENERANTA"),
    names(df)
  )

  if(length(marker_col) == 0){
    stop("❌ Brak kolumny z nazwami markerów")
  }

  marker_col <- marker_col[1]

  # =========================================================
  # ===== 2. marker names (wiersze)
  # =========================================================
  ...
  # ===== 3. dane (osobniki)
  # =========================================================
  mat <- df[, setdiff(names(df), marker_col), drop=FALSE]

  # =========================================================
  # ===== 4. numeric (lekko, bez kombinacji)
  # =========================================================
  ...
  # =========================================================
  # ===== 7. ID = nazwy kolumn (czyli osobniki)
  # =========================================================
  out$ID <- rownames(out)

  # 🔥 TYLKO to — bez niszczenia ID
  out$ID <- trimws(out$ID)

  # 🔥 usuń końcówkę A (bo masz 1_155A vs 1_155)
  out$ID <- sub("A$", "", out$ID)
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("OPTIMIZATION",
                 ...
                 plotlyOutput("pareto")),   # ✅ TO BYŁO BRAKUJĄCE
        ...
        tabPanel("AUTO FEATURES",
                 actionButton("gen_features","Generate Features"),
...
output$shap_dynamic <- renderPlotly({
  ...
  # 🔥 fallback jeśli brak wyboru
  if(is.null(col) || col == "" || !(col %in% colnames(rv$X))){
    col <- colnames(rv$X)[1]
  }
  
  if(!(col %in% colnames(rv$shap))) return(empty_plot())
  ...
  if(sum(ok) < 5){
    showNotification("⚠️ Too few SHAP points", type="warning")
    return(empty_plot())
  }
  ...
  # 🔥 jitter (usuwa nakładanie punktów)
  df$x <- jitter(df$x, factor = 0.1)
  ...
  # 🔥 dodaj trend line jeśli istnieje
...
vars_idx <- if(length(input$cf_vars) > 0){
  match(input$cf_vars, colnames(X))
} else {
  seq_along(x)
}
...
for(j in vars_idx){
  ...
  p1 <- tryCatch(
    predict(model, matrix(x_up,nrow=1)),
    error=function(e) NA
  )

  if(is.finite(p1)){
    grad[j] <- (p1 - pred0) / 1e-4
  }
}
...
for(j in seq_along(x)){
  if(!is.finite(x[j])) x[j] <- 0
      r <- safe_range(X[,j])
      x[j] <- max(min(x[j], r[2]), r[1])
    }

    pred_new <- tryCatch(predict(model, matrix(x,nrow=1)), error=function(e) NA)

    if(is.finite(pred_new)){
      loss <- (pred_new-target)^2
      if(loss < best_loss){
        best_loss <- loss
        best_x <- x
      }
  ...
  delta <- best_x - as.numeric(X[i,])

rv$cf <- data.frame(
  Feature = colnames(X),
  Original = as.numeric(X[i,]),
  Counterfactual = as.numeric(best_x),
  Change = delta,
  AbsChange = abs(delta),
  stringsAsFactors = FALSE
)
...
observeEvent(input$gen_features,{
  ...
  # usuń kolumny bez zmienności
  num <- num[, apply(num,2,function(x) sd(x,na.rm=TRUE)>0), drop=FALSE]
  
  if(ncol(num) < 2){
    showNotification("❌ Not enough numeric features", type="error")
    return()
  }
  ...
  if(length(top) < 2){
    ...
    showNotification("⚠️ Fallback to numeric features", type="warning")
  }
  ...
  for(i in 1:(length(top)-1)){
    for(j in (i+1):length(top)){
      ...
      new_feats[[paste0(top[i],"_BIO_",top[j])]] <- a * b
      new_feats[[paste0(top[i],"_DIFF_",top[j])]] <- a - b
  ...
  # usuń stałe
  keep <- apply(new_df,2,function(x) sd(x,na.rm=TRUE)>1e-8)
  new_df <- new_df[, keep, drop=FALSE]
  
  # usuń silnie skorelowane
  if(ncol(new_df) > 2){
    corr <- cor(new_df, use="pairwise.complete.obs")
    corr[!is.finite(corr)] <- 0
    
if(requireNamespace("caret", quietly = TRUE)){
  high_corr <- caret::findCorrelation(corr, cutoff=0.95, names=TRUE)
} else {
  high_corr <- character(0)
}

if(length(high_corr) > 0){
  new_df <- new_df[, !(colnames(new_df) %in% high_corr), drop=FALSE]
}  }
